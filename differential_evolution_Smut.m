...
function [xbest,fbest,count]=differential_evolution_Smut(func,n,low_l,up_l,max_call,pop_n,F,CR,nc)
d=up_l-low_l; % range
...
n_print=0;
print_idx=0;
while SD_max>0.0001 && count<max_call && SD_f>0.0001
    ...
    for i=1:pop_n
        ...
        [x_cb,f_min,count1]=s_algorithm_de(pop(i,:),r,n_choice,func,low_l,up_l);
        count=count+count1;
        xm=xbest+F*(x_cb-pop(i,:))+2*F*(xr(2,:)-xr(3,:));
...
% s for DE
function [x_cb,f_min,count]=s_algorithm_de(x_cb,r,n_choice,func,low_l,up_l)
f_min=func(x_cb);
count=0;
for idx=n_choice
    P=x_cb;
